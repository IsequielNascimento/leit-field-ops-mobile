 ...
 *
 * Every migration step must use CREATE-IF-NOT-EXISTS/ALTER-only statements
 * and must only be considered applied once `PRAGMA user_version` is advanced
 * inside the same transaction as its schema changes, so a failure never
 * leaves the database on a version it did not actually reach.
 */
export async function runMigrations(db: SQLiteDatabase): Promise<void> {
  ...
  throw new Error(`No migration path defined from schema version ${currentVersion} to ${SCHEMA_VERSION}.`);
}
